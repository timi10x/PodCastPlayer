...
class RssFeedService : FeedService {
    ...
    private fun domToRssFeedResponse(node: Node, rssFeedResponse: RssFeedResponse) {
        //i'm checking if the nodeType to make sure it's an XML element
        if (node.nodeType == Node.ELEMENT_NODE) {
            //store the node name and parent name i'm using this to determine where the current node resides in the tree
            ...
            if (parentName == "item" && grandParentName == "channel") {
                ...
                if (currentItem != null) {
                    //i wanna use the when expression to switch on the nodeName
                    when (nodeName) {
            ...
            //i'm assigning the nodelist to the list of child nodes for the current node
            val nodeList = node.childNodes
            //then for each child node, i call the domToRss...(), passing the
            //existing rssFeedR... object. THis allows domToR...() to keep building out the rssFeed... object
